...
class SegmentCleaner:
    def generate_cleaned_segments(self, input_path, output_path, repo_root):
        """
        ...
        return df_parquet_clean

    def load_cleaned_segments(self, file_path):
        segments_cleaned = pd.read_parquet(file_path)
        return segments_cleaned
    
    def get_cleaned_segments(self):
        """
        Returns the cleaned segments DataFrame.
        
        Returns:
            DataFrame: Cleaned segments data
        """
        
        if os.path.exists(file_path):
            self.segments_cleaned = self.load_cleaned_segments(file_path)
            print(f"LOG: Loaded cleaned segments from {file_path}")
        else:
            self.segments_cleaned = self.generate_cleaned_segments(
                input_path=repo_root / 'data' / 'raw' / 'reunion_segments.parquet',
                output_path=file_path,
                repo_root=repo_root
            )
            print(f"LOG: Generated and saved cleaned segments to {file_path}")
        return self.segments_cleaned
