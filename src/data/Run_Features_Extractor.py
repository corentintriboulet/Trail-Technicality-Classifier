...
from src.data.Run_Look_Up_Table_Generator import compute_segment_time_fast
...
class RunFeaturesExtractor:
    ...
    def cleaning_features_dataframe(self, df):
        df = df.copy()
    ...
    def get_features_dataframe_cleaned(self, df):
        features_df = self.get_features_dataframe(df)
        df_cleaned = self.cleaning_features_dataframe(df)
        return features_df, df_cleaned